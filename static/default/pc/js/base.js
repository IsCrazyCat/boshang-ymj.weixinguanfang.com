...
function loading(){
    ...
    if($(".ajax-box").length  ==  0){
        $(window.parent.document).find("body").append(boxHtml);
    }
...
function LoginSuccess(){
    $(window.parent.document).find(".ajax-box").show(); 
    $(window.parent.document).find(".ajax-dialog").remove();
...
function ipCall(){
	$.ajax({
		type: "post",
		url: "/index.php?m=cloud&a=ipfrom",
		dataType: "json",
		success: function (data) {
			if(data.site_id != null){
　			　if( window.top == window.self ){
				$('body').append('<div id="ip-tips" class="alert layout alert-yellow fixed-bottom ip-tips"><span class="close rotate-hover"></span><strong>位置提示：</strong>根据系统判断您不在这个城市，我们推荐您访问 [<a href="http://'+data.site_url+'">'+data.site_name+'</a>] 获得更多、更精确的服务！</div>');
　			　}
			}
		}
	});
...
function ajaxLogin(){
    hidde();
    var boxHtml = '<div class="dialog-mask"></div><div class="ajax-dialog"></div>';
    if($(".ajax-dialog").length  ==  0){
        $(window.parent.document).find("body").append(boxHtml);
        $(window.parent.document).find(".ajax-dialog").css('height',document.body.scrollHeight + 'px');
    }
    var url = '/index.php?g=pchome&m=passport&a=ajaxloging1&t='+Math.random();    
    var width = document.body.clientWidth
    $.get(url,function(data){
       
        $(window.parent.document).find(".ajax-dialog").html('<div class="ajax-dialog-back"></div>'+data);
        var left =  (width - 616)/2;
        var top = $(window).scrollTop() + 200;
		$(window.parent.document).find(".ajax-login").css({
            'left':left+'px',
            'top':top+'px'
            });
        $(window.parent.document).find(".dialog-mask").remove();
        $(window.parent.document).find(".ajax-dialog").show();
    },'html');    
...
function success(msg,timeout,callback){
    var boxHtml = '<div class="dialog-mask"></div><div class="ajax-box"></div>';
    if($(window.parent.document).find(".ajax-box").length  ==  0){
       $(window.parent.document).find("body").append(boxHtml);
    }
    
    $(window.parent.document).find(".ajax-box").html('<div class="alert alert-green radius-small"><i class="icon icon-check-circle"></i> <strong>恭喜：</strong>'+msg+'</div>');
    $(window.parent.document).find(".ajax-box").show();
    setTimeout(function(){
        lock = 0;
		$(window.parent.document).find(".dialog-mask").remove();
        $(window.parent.document).find(".ajax-box").hide();
        eval(callback);
...
function error(msg,timeout,callback){
    ...
    $(window.parent.document).find(".ajax-box").html('<div class="alert alert-red radius-small"><i class="icon icon-exclamation-circle"></i> <strong>注意：</strong>'+msg+'</div>');
    $(window.parent.document).find(".ajax-box").show();
    setTimeout(function(){
        lock = 0;
		$(window.parent.document).find(".ajax-dialog").remove();
...
$(function(e){
	...
	$pintuercheck=function(element,type,value){
		...
		switch(type){
			...
			case "mobile":return /^((\(\d{3}\))|(\d{3}\-))?13[0-9]\d{8}?$|15\d{9}?$|170\d{8}?$18\d{9}?$||147\d{8}?$/.test($pintu);break;
	...
	$(".spy a").each(function(){
		...
		thistarget.bind("scroll",function(){
			if(target==null){
				thistop=$(e.attr("rel")).offset().top - $(window).scrollTop() - parseInt(top);	
			}else{
				thistop=$(e.attr("rel")).offset().top - thistarget.offset().top - parseInt(top);
			};
	...
	$(".spy a").click(function(){
		var e = $(this);
		var t = e.closest(".spy");
		var x = t.attr("data-offset-nav");
		var y = $(e.attr("rel")).offset().top;
		var h = e.height();
		var n = y-h-x;
		$('html,body,.content').animate({scrollTop:n},300); 
	});
	...
	$.get("/mall/ajaxcart/t/"+t+".html", function (data) {
		$("#cart-num").html(data);
	}, 'html');
	...
	/*延时加载*/
	$("img.lazy").lazyload({placeholder:"/static/default/pc/image/load-w.png",effect:"fadeIn",threshold:300,failure_limit:10});  
    ...
    $(document).on('click','.yzm_code',function(){
        $("#"+$(this).attr('rel')).attr('src','/index.php?g=app&m=verify&a=index&mt='+Math.random());
    });
	...
	/*购买算法*/
     $(document).on("click","a[mini='buy']",function(e){
        e.preventDefault();
        if(!lock){
            loading();
	...
	/*团购下单*/
    $(document).on("click","a[mini='tuan']",function(e){
        e.preventDefault();
        if(!lock){
            lock=1;
            var url = $(this).attr('href');
    ...
    $(document).on("click","a[mini='load']",function(e){ //前台的MINILOAD 重构了
        e.preventDefault();
        var boxHtml = '<div class="ajax-dialog"></div>';
        if($(".ajax-dialog").length  ==  0){
            $("body").append(boxHtml);
            $(".ajax-dialog").css('height',document.body.scrollHeight + 'px');
        }
        if(!lock){
            loading();
            ...
            $.get(href,function(data){
                ...
                if(data == 0){
                    ...
                    $(".ajax-dialog").html('<div class="ajax-dialog-back"></div>'+data);
                    $(".ajax-dialog").show();
                }                
            },'html');
        }
...
$(document).on("click", "#ip-tips .close", function () {
	$("#ip-tips").remove();
});
