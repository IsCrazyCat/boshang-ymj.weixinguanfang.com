    ...
    private function parseTemplateName($templateName){
        ...
        foreach ($array as $templateName){
            if(false === strpos($templateName,$this->config['template_suffix'])) {
                ...
                if(defined('NOW_PATH') && !strpos($templateName,'@') &&  NOW_PATH != THEME_PATH){
                    $templateName1 = NOW_PATH.$module.C('TMPL_FILE_DEPR').$action.$this->config['template_suffix'];
                    if(file_exists($templateName1)) $templateName = $templateName1;
                }
            }
            // 获取模板文件内容
