...
class WxPayApi
{
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsNotify_urlSet()) {
            $inputObj->SetNotify_url(WEIXIN_NOTIFY_URL);
            //异步通知url
        }
        $inputObj->SetAppid(WEIXIN_APPID);
        //公众账号ID
        $inputObj->SetMch_id(WEIXIN_MCHID);
        //商户号
    ...
    private static function reportCostTime($url, $startTimeStamp, $data)
    {
        ...
        //如果仅失败上报
        if (WEIXIN_REPORT_LEVENL == 1 && array_key_exists("return_code", $data) && $data["return_code"] == "SUCCESS" && array_key_exists("result_code", $data) && $data["result_code"] == "SUCCESS") {
            return;
        }
        //上报逻辑
